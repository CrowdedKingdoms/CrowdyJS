...
export function formatUsdFromCents(
  ...
  const code = (currency ?? 'usd').trim().toLowerCase() || 'usd';
  if (!Number.isFinite(cents)) return `${balanceCents.trim()} ${code}`;
  try {
...
export function formatWalletBalanceLabel(wallet: FundsWalletSnapshot): string {
  return `wallet ${formatUsdFromCents(wallet.balanceCents, wallet.currency)}`;
}

export function formatWalletNeedsFundsLabel(
  wallet: FundsWalletSnapshot,
): string {
  return `${formatWalletBalanceLabel(wallet)} · top up to run`;
}
