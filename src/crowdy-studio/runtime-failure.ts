...
export const RUNTIME_FAILURE_WORKFLOW = [
  'Prefer a tight edit on the enclosing BEGIN_BLOCK when hashes are present; otherwise edit the file.',
  'After the write, the human will Test draft and re-Invoke.',
  ...
  if (source && typeof source === 'string' && source.length > 0) {
    ...
    if (block) {
      ...
      parts.push(
        ...
        'END_BLOCK',
        '',
        'Return a fixed version of ONLY the BEGIN_BLOCK...END_BLOCK region via write/edit.',
      );
    }
  }
